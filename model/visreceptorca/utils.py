...
class NeighborEmbedding(MessagePassing):
    ...
    def forward(self, data, x, edge_weight, edge_attr):
        ...
        # print(x_neighbors.dtype,'x_neighbors.dtype')
...
class ExpNormalSmearing(nn.Module):
    ...
    def forward(self, dist):  # 把距离变成径向分布函数个数的维度
        ...
        # print(dist.shape,'dist.shape',dist.dtype,'dist.dtype')
        ...
        output = self.cutoff_fn(dist) * torch.exp( -self.betas  * (torch.exp((-dist)) - self.means) ** 2)
        return output
...
class Distance(nn.Module):
    ...
    def forward(self, data):
        ...
        edge_index, pos, pos_CA = data[f"edge_index"], data[f"pos"], data[f"pos_CA"]

        edge_vec = pos[edge_index[0]] - pos[edge_index[1]]
        edge_vec_CA = pos_CA[edge_index[0]] - pos_CA[edge_index[1]]


        mask = edge_index[0] != edge_index[1]  # 非自循环的边
        # print(mask)
        edge_weight = torch.zeros(edge_vec.size(0), device=edge_vec.device)
        # print(edge_weight.type(),edge_vec.type())
        # print(edge_weight.type(),edge_vec.type())
        ...
        if self.return_vecs:
            ...
            edge_vec_CA = edge_vec_CA[lower_mask]
            return edge_index, edge_weight.clamp(1e-8), edge_vec, edge_vec_CA
