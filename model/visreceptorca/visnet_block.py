...
class ViSNetBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.lmax == 1:
            self.edge_proj = nn.Linear(6, 3)
        elif self.lmax == 2:
            self.edge_proj = nn.Linear(16, 8)
    ...
    def forward(self,
        ...
        edge_index, edge_weight, edge_vec, edge_vec_CA = self.distance(data)  
        # 获取边id，边长度，边向量（3维坐标差）
        assert (
        ...
        # print('径向分布函数的权重：',self.rbf_proj.weight[0])  
        # print(edge_attr.shape) # 78*256,78是边的数量，256是维数

        if self.bond_embedding is not None:
            edge_attr += self.bond_embedding(data)  #可以选择加上边的embedding

        # print(edge_vec,'edge_vector before')
        # print(torch.norm(edge_vec, dim=1)) #其实和edge_weight是一样的，都是边的长度
        edge_vec = edge_vec / torch.norm(edge_vec, dim=1).unsqueeze(1).clamp(min=1e-8)  # 重新归一化距离向量，具体做法是先求范数，然后除以范数，并且结果不能为0，所以加了一个很小的数
        edge_vec_CA = edge_vec_CA / torch.norm(edge_vec_CA, dim=1).unsqueeze(1).clamp(min=1e-8)
        # print(edge_vec.shape,'edge_vec shape')
        edge_vec = self.sphere(edge_vec)# 球谐函数操作,但lmax=1，所以这里没有什么变化，如果=2可以把3维变成8维
        edge_vec_CA = self.sphere(edge_vec_CA)
        # print(edge_vec,'edge_vec shape after sphere')
        # combine edge_vec and edge_vec_CA to new edge_vec, use linear layer to reduce dimension
        # print(torch.cat((edge_vec, edge_vec_CA), dim=1).shape,'edge_vec shape after cat')

        edge_vec = self.edge_proj(torch.cat((edge_vec, edge_vec_CA), dim=1))


        if self.neighbor_embedding is not None:
            # print('neighbor_embedding',x.dtype,edge_index.dtype,edge_weight.dtype,edge_attr.dtype,'x.dtype,edge_index.dtype,edge_weight.dtype,edge_attr.dtype')
            x = self.neighbor_embedding(data, x, edge_weight, edge_attr)   
            # 定义新的messagepassing，该函数按照有边连接的原子类型计算新的原子特征
        ...
        # 定义新的messagepassing，该函数按照有边连接的原子类型计算新的边特征，（原子1+原子2）*edge_attr
        
        for attn in self.attention_layers[:-1]:
            # print('attn',attn)
            dx, dvec, dedge_attr = attn(x, vec, edge_index, edge_weight, edge_attr, edge_vec)  
            # 输出的是原子特征，原子vec和边特征的变化量
            # 原子特征的变化量主要和qkv有关，也结合了vec_dot，vec_dot是vec先变成3维，再将前两维逐个相乘再求和
            # 原子vec主要和边向量有关，但也结合了vec_dot，
            # 边特征的变化量主要和vec在边向量上的投影有关，也结合了原来的边属性
            x = x + dx
            vec = vec + dvec
...
class EquivariantMultiHeadAttention(MessagePassing):
    ...
    def message(self, q_i, k_j, v_j, vec_j, dk, dv, r_ij, d_ij):  #i是源节点，j是目标节点
        ...
        # print(v_j.dtype, vec_j.dtype, d_ij.dtype,'v_j, vec_j, d_ij')
